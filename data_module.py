...
class MMDatasetQA(Dataset):
    def __init__(self, config, tokenizer, image_processor, data_path=None, max_length=512, split=None, processor=None, question_key="q", answer_key="a"):
        ...
        step_tokens = self.config.step_token.split(",")
        step_idx = self.config.step_idx.split(",")
        step_idx = [int(item) for item in step_idx]
        self.samples = []
        for line in self.data:
            if not isinstance(line['image'], list): continue
            prompt_steps = line['prompt_steps'][1:]
            prompt = [str(i+1) + ". " + prompt_steps[i] for i in step_idx]
            prompt = line['prompt_steps'][0] + "\n" + "\n".join(prompt)

            answer = [str(i+1) + ". " + line['reasoning_steps'][i].capitalize() for i in step_idx]
            answer = "\n\n".join(answer)
            answer = step_tokens[0] + answer + step_tokens[1] + "\n" + line['label']
#             answer =  step_tokens[1] + "\n" + line['label']
#             prompt = """You are a driving scene reasoning assistant.
# based on the provided multi-frame input from the ego vehicle’s front camera and relevant traffic rules, assessing whether any of objects pose a risk due to potential conflicts in trajectory, speed, or limited reaction time. conclude with a safety assessment."""

            if "type" in line.keys():
                if line['type'] != "real":
                    line['image'] = [line['image'][-1]]
                    
            self.samples.append(
                {
                    "image_path": line['image'][-4:],
        ...
        if "llava" in self.config.model_family:
            ...
            for ans in answers:
                if isinstance(image_path, list):
                    sources = [
                        {"role": "user", "content": [
                            {"type": "video", "video": image_path, "max_pixels": 600 * 400},
                            {"type": "text", "text": question}
                        ]},
                        {"role": "assistant", "content": [
                            {"type": "text", "text": ans}
