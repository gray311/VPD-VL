...
def main(cfg):
    ...
    elif "qwen" in cfg.model_id.lower():
        model = Qwen2_5_VLForConditionalGeneration.from_pretrained(cfg.model_id, torch_dtype=torch.bfloat16)
        processor = AutoProcessor.from_pretrained(cfg.model_id)
        image_processor = processor.image_processor
        tokenizer = processor.tokenizer
    
        if cfg.loss_type == "KL":
            oracle_model = Qwen2_5_VLForConditionalGeneration.from_pretrained(cfg.model_id, torch_dtype=torch.float16)
        
        if cfg.LoRA.r != 0:
            target_modules=r'.*model.*\.(up_proj|k_proj|down_proj|v_proj|q_proj|o_proj|gate_proj)'

    # SPECIAL_TOKENS = cfg.step_token.split(",")
    ...
    if cfg.LoRA.r != 0:
        ...
        for n, p in model.named_parameters():
            if cfg.tune_vision_tower and ("vision_model" in n or "visual.merger" in n):
        ...
        for n, p in model.named_parameters():
            if not cfg.tune_vision_tower and ("vision_model" in n or "visual.merger" in n or "visual.patch_embed" in n):
    ...
    #     batch['pixel_values_videos'] = batch['pixel_values_videos'].to(dtype=model.dtype)
    ...
    accelerator.init_trackers(project_name="visual_prompt_driving")
