...
NUPLAN_DATA_ROOT = os.getenv('NUPLAN_DATA_ROOT', '/data/yingzi_ma/Visual-Prompt-Driving/workspace/nuplan/dataset')
NUPLAN_MAPS_ROOT = os.getenv('NUPLAN_MAPS_ROOT', '/data/yingzi_ma/Visual-Prompt-Driving/workspace/nuplan/dataset/maps')
NUPLAN_DB_FILES = os.getenv('NUPLAN_DB_FILES', "/data/yingzi_ma/Visual-Prompt-Driving/workspace/nuplan/dataset/nuplan-v1.1/splits/mini")
...
class NuPlanScenarioLoader:
    ...
    def get_ego_information(self, iteration):
        ...
        image_filename = os.path.join(image_save_path, f"{str(self.frame_num).zfill(6)}.png")
        ...
        object_infos = self.object_infos[int(timestamp / 500000)]
        object_descriptions = {}
        object_num = 0
        ...
        for lane_id in lane_ids:
            ...
            if lane_polygon is None: continue
        ...
        for lane_connector_id in lane_connector_ids:
            ...
            lane_polygon = self._get_map_segmentation(lane_connector, ego_pose, camera, intersection_threshold=200, connector=True)
            if lane_polygon is None: continue
        ...
        for item in tqdm(self.log_db.image):
            ...
            if DEFAULT_SENSOR_CHANNEL in item.filename_jpg:
                ...
                object_infos[timestamp] = {item[0]: {
                    ...
                    "ego_pose": item[5],
                    "camera": item[6]} for i, item in enumerate(object_info)}
