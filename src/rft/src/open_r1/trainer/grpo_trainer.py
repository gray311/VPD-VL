...
class VLMGRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for prompt in prompts:
            del prompt[0]['content'][0]['text']
            del prompt[0]['content'][1]['video']
            processed_prompts.append(prompt)
            
        prompts_text = self.processing_class.apply_chat_template(
            processed_prompts, tokenize=False, add_generation_prompt=True
        )
        ...
        for x in inputs:
            ...
            for img in imgs:
                try:
                    ...
                    img = img.resize((600, 400), PIL.Image.Resampling.LANCZOS)
                except:
                    pass
        ...
        # driving-related information is needed for the reward function
        if "object" in inputs[0].keys():
            additional_output = [{"object": x['object'], "behavior": x['behavior'], "label": x['label']} for x in inputs]
