...
class GRPOScriptArguments(ScriptArguments):
    ...
    media_type: Optional[str] = field(
        default=None,
        metadata={"help": "Choose task type: 'video', 'image', ..."},
    )
            ...
            for line in f:
                ...
                if 'image' in item:
                    if isinstance(item['image'], str):
                        ...
                        item['image_path'] = [os.path.join(image_folder, image.replace("./", "")) for image in item['image']]
                        del item['image'] # remove the image column so that it can be loaded later
                    else:
                        raise ValueError(f"Unsupported image type: {type(item['image'])}")
                    
                step_idx = [0,1,2,3]
                step_tokens = ["<think>", "</think>"]

                fixed_prompt_steps = [
                    "Key Dynamic Objects\nin this step， plesae list only dynamic objects that may affect/be affected by the ego; exclude the ego. Produce a fixed header plus one line per object. For example, your output should follow this format:\nobject 1 — <position>, <type>. <one concise sentence>\nobject 2 — <position>, <type>. <one concise sentence>\nobject 3 — ...\n\nFormatting rules:\n- First line MUST be exactly: \"Key Dynamic Objects  \" (two spaces at the end), then a newline.\n- Each line starts with \"object <k> —\" (em dash U+2014), then <position>, \", \", <type>, \". \", and ONE concise sentence; end with a period.\n- Use compact position phrases (e.g., ahead-left/right side/far left/behind/oncoming) and type phrases (e.g., white sedan/blue bus/pedestrian).\n- No extra lines, bullets, or commentary.",
                    "Driving Decision Prediction\nin this step， plesae redict immediate (≈1–3 s) longitudinal and lateral behaviors for the ego and EVERY object from Step 2; ego first, then objects by index. For example, your output should follow this format:\nego: <longitudinal>, <lateral>\nobject 1: <longitudinal>, <lateral>\nobject 2: <longitudinal>, <lateral>\nobject 3: ...\n\nTokens — longitudinal: decelerate | accelerate | keep speed | stop | yield | follow | reverse | park | unknown; lateral: keep lane | change lane left | change lane right | turn left | turn right | merge left | merge right | u-turn | overtake | park | yield | unknown.\nNo explanations or extra lines.",
                ]

                prompt_steps = item['prompt_steps'][1:]
                prompt_steps[1] = fixed_prompt_steps[0]
                prompt_steps[2] = fixed_prompt_steps[1]


                prompt = [str(i+1) + ". " + prompt_steps[i] for i in step_idx]
                prompt = item['prompt_steps'][0] + "\n\n" + "\n\n".join(prompt)

                answer = [str(i+1) + ". " + item['reasoning_steps'][i].capitalize() for i in step_idx]
                answer = "\n\n".join(answer)
                answer = step_tokens[0] + answer + step_tokens[1] + "\n" + item['label']
                ...
                line['object'] = item['attribute']['object']
                line['behavior'] = item['attribute']['behavior']
                line['accu_reward_method'] = item.get('accu_reward_method', accu_reward_method) # if accu_reward_method is in the data jsonl, use the value in the data jsonl, otherwise use the defined value
                line['image_path'] = item['image_path']
                line['label'] = item['label']
                if "type" in item.keys():
                    line['type'] = item['type']
                else:
                    line['type'] = "real"
                if 'nexar' not in item['id']: continue
    ...
    def make_conversation_from_jsonl(example):
        if 'image_path' in example and example['image_path'] is not None:
            ...
            if example['type'] == "counterfactual":
                example['image_path'] = example['image_path'][-1:] 
            else:
                example['image_path'] = example['image_path'][-4:] 
            ...
            return {
                ...
                'prompt': [{
                    'role': 'user',
                    'content': [
                        {'type': 'video', 'video': example['image_path']},
